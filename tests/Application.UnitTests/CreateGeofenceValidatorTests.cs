...
public class CreateGeofenceValidatorTests
{
    ...
    public void Should_Have_Error_When_Name_Is_Empty()
    {
        ...
        var command = new CreateGeofenceCommand(new GeofenceDto(Guid.NewGuid(), geom, "", null, 1, 1, true));
    ...
    {
        var command = new CreateGeofenceCommand(new GeofenceDto(Guid.NewGuid(), default, "Name", null, 1, 1, true));
        var result = _validator.TestValidate(command);
    ...
    public void Should_Have_Error_When_Coordinates_Are_Empty()
    {
        ...
        var command = new CreateGeofenceCommand(new GeofenceDto(Guid.NewGuid(), geom, "Name", null, 1, 1, true));
        var result = _validator.TestValidate(command);
    ...
    {
        var coords = new[] { new CoordinateVm(10.5, 20.5), new CoordinateVm(11.0, 21.0), new CoordinateVm(10.5, 20.5) };
        var geom = new MultiPolygonVm(coords, 4326);
        var command = new CreateGeofenceCommand(new GeofenceDto(Guid.NewGuid(), geom, "Warehouse Zone", "Main warehouse", 1, 1, true));
        var result = _validator.TestValidate(command);
